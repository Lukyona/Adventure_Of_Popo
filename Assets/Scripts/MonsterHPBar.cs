...
public class MonsterHPBar : MonoBehaviour
{
    ...
    void Update()
    {
        if(Player.instance.CombatComponent.target != null && !targetIn)//몬스터가 타겟으로 지정됐을 때
        {
            targetIn = true;
            for (int i = 0; i < transformtList.Count; i++)
            {
                if(Player.instance.CombatComponent.target.transform == transformtList[i].transform)//타겟인 몬스터 찾고 일치하면
                {
                    color.a = 255f;
                    color1.a = 255f;
                    hpBarList[i].GetComponent<Image>().color = color; //hp바를 보이게 만듬
                    levelList[i].GetComponent<Image>().color = color1;//레벨 정보
                    if (Player.instance.CombatComponent.target.name.Contains("Slime") || Player.instance.CombatComponent.target.name.Contains("Turtle"))
                    {
                        levelList[i].transform.GetChild(0).GetComponent<Text>().text = 1.ToString();
                    }
                    else if (Player.instance.CombatComponent.target.name.Contains("Log"))
                    {
                        levelList[i].transform.GetChild(0).GetComponent<Text>().text = 2.ToString();
                    }
                    else if (Player.instance.CombatComponent.target.name.Contains("Bat"))
                    {
                        levelList[i].transform.GetChild(0).GetComponent<Text>().text = 3.ToString();
                    }
                    else if (Player.instance.CombatComponent.target.name.Contains("Mushroom") || Player.instance.CombatComponent.target.name.Contains("Dog"))
                    {
                        levelList[i].transform.GetChild(0).GetComponent<Text>().text = 4.ToString();
                    }
                    else if (Player.instance.CombatComponent.target.name.Contains("Dragon"))
                    {
        ...
        if(Player.instance.CombatComponent.target == null && targetIn)
        {
            DisappearMonsterInfo();
        }

        if (Player.instance.CombatComponent.target != null)//타겟 있을 때만 화살표,hp바,레벨 정보가 몬스터 따라다니게
        {      
            if (Player.instance.CombatComponent.target.name.Contains("Dog"))//문지기
            {
            ...
            else if(Player.instance.CombatComponent.target.name.Contains("Dragon"))//보스
            {
    ...
    {
        if(GameDirector.instance.mainCount == 10 && Player.instance.CombatComponent.target == null)//타겟 없지만 보스전일 때
        {
            hpBarList[0].GetComponent<Image>().fillAmount -= dam / MonsterHPBar.instance.boss.GetComponent<IEnemyController>().GetMaxHealth();
        }
        else//보스전 제외 모두
        {
            if (Player.instance.CombatComponent.target.name.Contains("Turtle"))//거북이만 유일하게 입는 데미지 다름
            {
            ...
            else
            {
                ...
                {
                    hpBarList[num].GetComponent<Image>().fillAmount -= dam / Player.instance.CombatComponent.target.GetComponent<IEnemyController>().GetMaxHealth();//몬스터의 본래 체력에 따라 체력바 각각 다르게 감소  
                }
                else
                {
                    hpBarList[num].GetComponent<Image>().fillAmount -= dam / Player.instance.CombatComponent.target.GetComponent<IEnemyController>().GetMaxHealth();
                }
            }
        }           
    }
    ...
    public void DisappearMonsterInfo()//기존 타겟 정보 안 보이게
    {
        ...
        if (Player.instance.CombatComponent.target != null)//타겟 죽었을 경우
        {
            if((GameDirector.instance.mainCount < 9 && Player.instance.CombatComponent.target.GetComponent<IEnemyController>().IsDead())
                || (GameDirector.instance.mainCount >= 9 && Player.instance.CombatComponent.target.GetComponent<IEnemyController>().IsDead()))
            {
