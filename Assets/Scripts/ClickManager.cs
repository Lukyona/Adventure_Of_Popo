    ...
    private void Update()
    {
        if(Input.GetMouseButtonDown(0))
        {
            ...
            {//범위 11, 클릭한 오브젝트 태그가 몬스터이고 몬스터가 플레이어 시야 내에 있을 때, 플레이어가 죽은 상태가 아닐 때
                ...
                Player.instance.CombatComponent.target = targetObj;//타겟 할당
                PlayerInfoManager.instance.monster = targetObj;
            }
            if(Physics.Raycast(ray, out hit, 20f) && !(hit.collider.gameObject.tag == "Monster") && Player.instance.CombatComponent.target != null)//타겟이 설정된 상태에서 클릭한 게 몬스터가 아니면
            {
