...
{
    private int GetNumber(string name, int nameSize)
    {
    ...
    private void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(ray, out hit, 11f) && hit.collider.gameObject.tag == "Enemy" && Player.instance.CombatComponent.EnemyInTargetRange && !Player.instance.IsDead())
